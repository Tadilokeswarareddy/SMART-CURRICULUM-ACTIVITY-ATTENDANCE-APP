        ...
        <div style={s.banner}>
          ...
          <div style={s.bannerInner}>
            ...
            <div style={{ animation: "fadeUp 0.45s ease both", animationDelay: "0.08s" }}>
              <span style={s.roleTag}>Faculty</span>
              <h1 style={s.heroName}>{profile.full_name}</h1>
              <span style={s.pill}>{profile.designation || "Teacher"}</span>
            </div>
          </div>
          ...
          <div style={{ ...s.card, animation: "fadeUp 0.5s ease both", animationDelay: "0.2s" }}>
            ...
            {profile.bio && (
              <div style={{ marginTop: 24 }}>
                ...
                <div style={s.bioBox}>{profile.bio}</div>
              </div>
            )}
          </div>
          ...
          {profile.subjects_taught?.length > 0 && (
            <div style={{ ...s.card, marginTop: 20, animation: "fadeUp 0.5s ease both", animationDelay: "0.32s" }}>
              ...
              <div style={s.subjectsGrid}>
                {profile.subjects_taught.map((item, i) => (
                  <div
                    key={i}
                    className="subject-card"
                    style={s.subjectCard}
                  >
                    <div style={s.subjectDot} />
                    <div>
                      <p style={s.subjectName}>{item.subject}</p>
                      <p style={s.subjectSec}>{item.section}</p>
                    </div>
                  </div>
                ))}
              </div>
...
const s = {
  ...
  bioBox: {
    ...
    borderRadius: 10, fontSize: 14, color: G[900],
    lineHeight: 1.7, whiteSpace: "pre-wrap",
    fontFamily: "'DM Sans', sans-serif",
  },

  subjectsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 12 },
  subjectCard: {
  ...
  subjectDot: {
    ...
    background: G[500], marginTop: 5, flexShrink: 0,
  },
  subjectName: { fontSize: 14, fontWeight: 600, color: G[800], margin: 0, fontFamily: "'DM Sans', sans-serif" },
  subjectSec:  { fontSize: 12, color: G[600], margin: "3px 0 0", fontWeight: 500, fontFamily: "'DM Sans', sans-serif" },
}
