  ...
  return (
    <>
      <style>{`
        ...
        .lp-forgot:hover { color:${G[800]} !important; text-decoration:underline; }
      `}</style>
      ...
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:G[50], padding:16, fontFamily:"'DM Sans',sans-serif" }}>
        <div style={{ width:"100%", maxWidth:860, background:"#fff", borderRadius:24, boxShadow:"0 8px 40px rgba(0,0,0,0.1)", overflow:"hidden", display:"flex", minHeight:500, animation:"fadeUp 0.5s ease both" }}>
          ...
          <div style={{ flex:"0 0 45%", background:`linear-gradient(135deg,${G[900]} 0%,${G[700]} 50%,${G[500]} 100%)`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 32px", position:"relative", overflow:"hidden" }} className="lp-left">
            ...
            <div style={{ animation:"fadeUp 0.5s ease both", animationDelay:"0.1s" }}>
              ...
              <form onSubmit={SendRequest} style={{ display:"flex", flexDirection:"column", gap:24 }}>
                <div>
                  ...
                  <input type="text" autoCapitalize="none" autoCorrect="off" autoComplete="username" spellCheck="false" placeholder="Enter your username" value={username} onChange={(e) => setUsername(e.target.value)} className="lp-input" />
                </div>
                <div>
                  ...
                  <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
                    <input type={showPassword ? "text" : "password"} autoCapitalize="none" autoCorrect="off" autoComplete="current-password" placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} className="lp-input" style={{ paddingRight:28 }} />
                    <button type="button" onClick={() => setShowPassword(p => !p)} className="lp-eye" style={{ position:"absolute", right:0, background:"none", border:"none", cursor:"pointer", color:"#9ca3af", padding:0, display:"flex", alignItems:"center", transition:"color 0.2s" }} aria-label={showPassword ? "Hide password" : "Show password"}>
                ...
                <div style={{ textAlign:"right" }}>
                  <span onClick={() => navigate("/forgot-password")} className="lp-forgot" style={{ fontSize:12, color:G[600], cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"color 0.2s" }}>
                    Forgot password?
                  </span>
                </div>

                <button type="submit" disabled={loading} className="lp-btn" style={{ background:G[800], color:"#fff", border:"none", borderRadius:12, padding:"13px", fontSize:14, fontWeight:600, fontFamily:"'DM Sans',sans-serif", cursor:"pointer", transition:"background 0.2s", opacity:loading?0.7:1 }}>
