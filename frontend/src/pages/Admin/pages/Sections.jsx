...
const Sections = ({ data, reload, toast }) => {
  ...
  const save = async () => {
    if (!form.branch_id || !form.year_id || !form.name?.trim()) { toast("All fields are required"); return }
    try {
      const payload = { branch: form.branch_id, year: form.year_id, name: form.name }
      modal === "edit"
        ? await api.patch(`/api/sections/${form.id}/`, payload)
        : await api.post("/api/sections/", payload)
      toast(modal === "edit" ? "Section updated" : "Section created"); close(); reload()
    ...
    <>
      ...
      <div style={s.card}>
        <CardHeader label="All Sections" count={items.length}>
          ...
          <button style={s.btnPrimary} onClick={() => open()}><Icon name="plus" size={14} />Add Section</button>
        </CardHeader>
        <TableWrap cols={["Section", "Branch", "Year", "Actions"]}>
          {items.length === 0 ? <EmptyRow cols={4} /> : items.map(sec => (
            <tr key={sec.id}>
              ...
              <Td><BtnRow>
                <button style={s.btnSm} onClick={() => open(sec)}><Icon name="edit" size={12} />Edit</button>
                <button style={s.btnDanger} onClick={() => del(sec.id)}><Icon name="trash" size={12} />Delete</button>
              </BtnRow></Td>
            </tr>
          ))}
        </TableWrap>
      ...
      {modal && (
        <Modal title={modal === "edit" ? "Edit Section" : "Add Section"} onClose={close} onSave={save}>
          <div style={s.formRow}>
            <Field label="Branch">
          ...
          <Field label="Section Name (A, B, C…)">
            <Input value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder="A" />
          </Field>
        </Modal>
      )}
    </>
