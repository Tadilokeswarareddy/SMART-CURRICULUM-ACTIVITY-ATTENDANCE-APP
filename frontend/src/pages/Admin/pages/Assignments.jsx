...
const Assignments = ({ data, reload, toast }) => {
  ...
  const save = async () => {
    if (!form.teacher_id || !form.subject_id || !form.section_id) {
    ...
    try {
      ...
      toast(modal === "edit" ? "Assignment updated" : "Assignment created")
      close(); reload()
    } catch (e) {
      console.log("Assignment error:", e?.response?.data)
      toast(e?.response?.data?.non_field_errors?.[0] || "Error — this assignment may already exist")
    }
  }
  ...
  const items = (data.assignments || []).filter(a =>
    `${a.teacher} ${a.subject?.name} ${a.section}`
  ...
  return (
    <>
        ...
        <TableWrap cols={["Teacher", "Subject", "Section", "Actions"]}>
          {items.length === 0 ? <EmptyRow cols={4} /> : items.map(a => (
            <tr key={a.id}>
              ...
              <Td>
                <BtnRow>
                  <button style={s.btnSm} onClick={() => open(a)}>
                    <Icon name="edit" size={12} />Edit
                  </button>
                  <button style={s.btnDanger} onClick={() => del(a.id)}>
      ...
      {modal && (
        <Modal
          title={modal === "edit" ? "Edit Assignment" : "New Assignment"}
          onClose={close}
          onSave={save}
        >
          <Field label="Teacher">
